...
<div class="flex space-x-4">
    <div class="mx-5 my-1 w-1/6">
        <div class="px-5 py-5 fixed z-10 border-2 border-primary rounded-md">
            <ol x-cloak x-data="{ formNav : 1 }"  class="overflow-hidden space-y-8">
                <li class="relative flex-1 after:content-['']  after:w-0.5 after:h-full  after:bg-primary after:inline-block after:absolute after:-bottom-11 after:left-4 lg:after:left-5">
                    <a href="#school_information" class="flex items-center font-medium w-full  ">
                        <span class="w-8 h-8 bg-primary border-2 border-transparent rounded-full flex justify-center items-center mr-3 text-sm text-white lg:w-10 lg:h-10">
                        ...
                        <div class="block">
                            <h4 class="text-lg  text-primary">Step 1</h4>
                ...
                <li class="relative flex-1 after:content-['']  after:w-0.5 after:h-full  after:bg-gray-200 after:inline-block after:absolute after:-bottom-12 after:left-4 lg:after:left-5">
                    <a href="#school_resources" class="flex items-center font-medium w-full  ">
                        <span class="w-8 h-8 bg-indigo-50  border-2 border-primary rounded-full flex justify-center items-center mr-3 text-sm text-primary lg:w-10 lg:h-10">2</span>
                        <div class="block">
                ...
                <li class="relative flex-1 ">
                    <a href="#appointments_fundings" class="flex items-center font-medium w-full hover:text-primary">
    ...
    <div class="max-w-5/6 w-5/6 lg:pe-8">
        <div x-cloak x-data="{ formNav : 1 }" class="flex space-x-8 justify-end">
            {{-- fixed flex flex-col top-12 left-0  --}}
            <div class="w-full">
                ...
                @csrf
                            ...
                            <div class="mt-5">
                                <div class="m-0 mb-4 p-0 flex space-x-3 justify-between">
                                    <span class="w-1/4">
                                        ...
                                        <x-input id="school_id" type="number" name="school_id" :value="$school->school_id ?? ''" :readonly="$school ?? false" required/>
                                    </span>
                                    <span class="w-1/4">
                                        <x-label for="region" value="{{ __('Region') }}"/>
                                        <x-input id="region" type="text" name="region" :value="$school->region ?? ''" :readonly="$school ?? false" required/>
                                    </span>
                                    <span class="w-1/4">
                                        <x-label for="division" value="{{ __('Division') }}" />
                                        <x-input id="division" type="text" name="division" :value="$school->division ?? ''" :readonly="$school ?? false" required/>
                                    </span>
                                    <span class="w-1/4">
                                        <x-label for="district" value="{{ __('District') }}" />
                                        <x-input id="district" type="text" name="district" :value="$school->district ?? ''" :readonly="$school ?? false" required/>
                                    </span>
                                </div>
                                <div class="mb-4 flex space-x-3 justify-between">
                                    <span class="w-full">
                                        <x-label for="school_name" value="{{ __('School Name') }}" />
                                        <x-input id="school_name" type="text" name="school_name" :value="$school->school_name ?? ''" :readonly="$school ?? false" required/>
                                    </span>
                                </div>
                                <div class="mb-4 flex space-x-4 justify-between">
                                    <span class="w-full">
                                        <x-label for="address" value="{{ __('Address') }}" />
                                        <x-input id="address" type="text" name="address" :value="$school->address ?? ''" :readonly="$school ?? false" required/>
                                    </span>
                                </div>
                                <div class="mb-4 flex space-x-3 justify-between">
                                    <span class="w-2/4">
                                        <x-label for="email" value="{{ __('Email') }}" />
                                        <x-input id="email" type="email" name="email" :value="$school->email ?? ''" :readonly="$school ?? false" required/>
                                    </span>
                                    <span class="w-2/4">
                                        <x-label for="phone" value="{{ __('Phone') }}" />
                                        <x-input id="phone" type="text" name="phone" :value="$school->phone ?? ''" :readonly="$school ?? false" required/>
                                    </span>
                                </div>
                                <div class="flex space-x-4">
                                    <div class="w-full">
                                        <x-label for="curricular_classification" value="{{ __('Curricular Classifications') }}" />
                                        <x-grade-level-multi-select/>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <section id="school_resources" class="mt-8 bg-white overflow-auto shadow-md sm:rounded-lg">
                        <div class="mx-auto pt-8 pb-10 px-8">
                            ...
                            <div class="m-0 p-0">
                                <livewire:dynamic-form.funded-item-form />
                            </div>
                        </div>
                    </section>
                    <section id="appointments_fundings" class="mt-8 bg-white overflow-auto shadow-md sm:rounded-lg">
                        <div class="mx-auto pt-8 pb-10 px-8">
                            ...
                            <div class="m-0 p-0">
                                <livewire:dynamic-form.appointments-funding-form />
                            </div>
                        </div>
                    </section>
