...
<x-dialog-modal wire:model.live="confirmingFundedItemDeletion">
    ...
    <x-slot name="footer">
        <x-button wire:click.prevent="$toggle('confirmingFundedItemDeletion')" wire:loading.attr="disabled">
            {{ __('Cancel') }}
        </x-button>

        <x-button class="ms-3" wire:click.prevent="deleteFundedItem" wire:loading.attr="disabled">
            {{ __('Delete Account') }}
        </x-button>
    </x-slot>
</x-dialog-modal>
